...
- [ ] 新会话会在 `~/.codia/projects/<project-id>/sessions/` 目录创建 JSONL 文件，文件名符合 `YYYYMMDD-HHMMSS-xxxx` 格式（验证：history 测试通过，并断言生成的会话文件路径与名称）
...
- [ ] 场景 3：完成一轮自然结束的对话后，系统异步更新 `~/.codia/projects/<project-id>/memory/` 或 `~/.codia/memory/` 下的记忆，再开启下一次新会话时可读到更新后的索引（验证：连续执行“对话结束 → 检查记忆文件 → 新会话读取”流程）
