...
// 格式：<project-root>/sessions/YYYYMMDD/YYYYMMDD-HHMMSS-xxxx.jsonl
export function newSessionPath(now?: Date, projectRoot?: string): string {
  ...
  const dateDir = join(getSessionsDir(projectRoot), dateStr);
...
function collectSessionFiles(sessionsDir: string): { name: string; path: string; stat: ReturnType<typeof statSync> }[] {
  const result: { name: string; path: string; stat: ReturnType<typeof statSync> }[] = [];
  try {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        result.push({
          ...
          stat: statSync(join(sessionsDir, entry.name)),
        });
      }
    }
  } catch {
